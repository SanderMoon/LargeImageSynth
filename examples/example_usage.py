...
def run_binary_example():
    ...
    hdf5_path = os.path.join(
        ...
        experiment_config.dataset_config.hdf5_filename
    )
    pipeline.save_to_hdf5(hdf5_path)
    
    print(f"✅ Binary example completed! Output saved to: {experiment_config.output_dir}")
...
def run_multiclass_example():
    ...
    print(f"✅ Multiclass example completed! Output saved to: {experiment_config.output_dir}")


def visualize_embeddings(annotations_path: str, embeddings_path: str, num_samples: int = 32):
...
def main():
    ...
    try:
        ...
        embeddings_path = os.path.join(config.output_dir, "synthetic_dataset_embeddings.csv")
