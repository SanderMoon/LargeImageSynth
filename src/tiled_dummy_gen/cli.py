"""Command-line interface for LargeImageSynth.

This module provides command-line tools for running synthetic data generation
pipelines, creating spatial learning datasets, and managing tiled image datasets.
"""
...
def _initialize_pipeline(
    config_path: Path, output_dir: Optional[Path], verbose: bool
):
    """Helper to load config and initialize the appropriate pipeline."""
    if verbose:
    ...
    # Try scene-based configuration first
    try:
        scene_loader = SceneConfigLoader(str(config_path))
        scene_config = scene_loader.load_config()
        
        if output_dir:
            scene_config.output_dir = str(output_dir)
            logger.info(f"Output directory overridden to: {output_dir}")
        
        logger.info("Using scene-based pipeline")
        return SceneBasedPipeline(scene_config)
        
    except Exception as e:
        logger.debug(f"Scene config loading failed: {e}")
        ...
        try:
            ...
            return SyntheticDataPipeline(config=experiment_config)
            
        except Exception as legacy_e:
            raise ValueError(f"Failed to load config as either scene format ({e}) or legacy format ({legacy_e})")


@click.group()
@click.version_option()
def main():
    """LargeImageSynth: Generate large synthetic images with spatial objects, tiling, and embeddings."""
    pass
