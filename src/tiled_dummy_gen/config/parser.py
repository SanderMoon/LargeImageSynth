...
class ExperimentConfigLoader:
    """
    A class to load a JSON configuration file, parse it, and build an ExperimentConfig instance.
    It handles class templates and variations to generate multiple ClassConfig instances.
    """

    def __init__(self, json_path: str):
        """
        Initializes the ExperimentConfigLoader with the path to the JSON configuration file.
    ...
    def _load_json(self) -> Dict[str, Any]:
        """
        Loads the JSON configuration file. Caches the result.
        """
    ...
    def get_experiment_config(self) -> ExperimentConfig:
        """
        Parses the JSON configuration and returns an ExperimentConfig instance. Caches the result.
        """
        if self._experiment_config is None:
            self._experiment_config = self._parse_experiment_config()
    ...
    def _parse_experiment_config(self) -> ExperimentConfig:
        ...
        try:
            class_template = config_data.get("class_template", {})
            ...
            num_tiles_base = config_data.get("num_tiles_base", 1)
            image_size = tuple(config_data.get("image_size", [224, 224]))
    ...
    def _parse_class_configs(
        ...
        for combination in itertools.product(*variation_values):
            ...
            logger.debug(f"Generated ClassConfig: {class_config}")
    ...
    def _parse_embedder_config(self, embedder_config_data: Dict[str, Any]) -> EmbedderConfig:
        ...
        try:
            ...
            logger.debug(f"Parsed EmbedderConfig: {embedder_config}")
    ...
    def _parse_dataset_config(self, dataset_config_data: Dict[str, Any]) -> DatasetConfig:
        ...
        try:
            ...
            logger.debug(f"Parsed DatasetConfig: {dataset_config}")
    ...
    def _parse_split_config(self, split_config_data: Dict[str, Any]) -> SplitConfig:
        ...
        try:
            ...
            logger.debug(f"Parsed SplitConfig: {split_config}")
            return split_config
        except KeyError as e:
